...
EnemyBeam::EnemyBeam()
	:GameObject(), hImage_(-1), pos_({ -10, -10 }), speed_(0), 
	imageSize_({ ENEMY_BEAM_IMAGE_WIDTH, ENEMY_BEAM_IMAGE_HEIGHT }), isFired_(true)
{
	hImage_ = LoadGraph("Assets\\ebeams.png"); // 弾の画像を読み込む
	speed_ = ENEMY_BEAM_INIT_SPEED; // 弾の移動速度を設定
	AddGameObject(this); // 弾オブジェクトをゲームオブジェクトのベクターに追加
}

EnemyBeam::EnemyBeam(float x, float y)
	:GameObject(), hImage_(-1), pos_({ x, y }), 
	speed_(ENEMY_BEAM_INIT_SPEED), imageSize_({ ENEMY_BEAM_IMAGE_WIDTH, ENEMY_BEAM_IMAGE_HEIGHT }), isFired_(true)
{
	hImage_ = LoadGraph("Assets\\ebeams.png"); // 弾の画像を読み込む
	AddGameObject(this); // 弾オブジェクトをゲームオブジェクトのベクターに追加
}
...
void EnemyBeam::Draw()
{
	...
	if (isFired_)
	{
		DrawExtendGraphF(pos_.x, pos_.y, pos_.x + imageSize_.x, 
			pos_.y + imageSize_.y, hImage_, TRUE); // 弾を描画
	}
}
